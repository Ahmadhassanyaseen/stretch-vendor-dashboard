...
/**
 * IMPORTANT NOTES ABOUT UBER FREIGHT API ACCESS:
 * 
 * 1. API ACCESS RESTRICTIONS:
 *    - Uber Freight API may require special approval and partnership agreements
 *    - Not all developer accounts have freight API access by default
 *    - You may need to apply for freight partner status
 * 
 * 2. CURRENT API STATUS:
 *    - Uber has been transitioning their freight APIs in 2024
 *    - New scheduling API standards have been implemented
 *    - Some endpoints may require different authentication flows
 * 
 * 3. AUTHENTICATION REQUIREMENTS:
 *    - Standard OAuth 2.0 client credentials flow
 *    - Scope: 'freight.loads' (as mentioned in Microsoft documentation)
 *    - May require additional business verification
 * 
 * 4. ENDPOINT STRUCTURE:
 *    - Production: https://api.uber.com/v1/freight/
 *    - Sandbox: https://sandbox-api.uber.com/v1/freight/
 *    - OAuth: /oauth2/token (standard Uber endpoint)
 */

/**
 * Uber Freight API Client for Load Posting
 * Handles authentication and load creation
 * Updated with correct Uber API endpoints and improved error handling
 */
class UberFreightAPI {
    ...
    public function __construct($clientId, $clientSecret, $sandbox = true) {
        ...
        // Correct Uber API endpoints based on official documentation
     ...
     * Updated with correct Uber Freight API endpoint structure
     */
    public function createLoad($loadData) {
        // Updated URL structure for Uber Freight API
        $url = $this->baseUrl . '/v1/freight/loads';
        
        // Validate required fields
        $this->validateLoadData($loadData);
        
        echo "Creating load at endpoint: {$url}\n";
...
class UberFreightEndpointTester {
    ...
    public function findWorkingEndpoint() {
        // Updated possible base URLs based on official Uber documentation
        $possibleUrls = [
            'https://sandbox-api.uber.com',      // Official Uber sandbox
            'https://api.uber.com',              // Production (use carefully)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['webhook'])) {
    ...
    $tester = new UberFreightEndpointTester('YOUR_CLIENT_ID', 'YOUR_CLIENT_SECRET');
