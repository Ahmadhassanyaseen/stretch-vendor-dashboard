...
async function formatDetails(data) {
    ...
    loadAdditionalDetails(details).then(updatedContent => {
        // Find the parent container and replace the loading state with the actual content
        const container = document.querySelector('.details-container');
        if (container) {
            container.outerHTML = updatedContent;
        }
    });
    // In the formatDetails function, modify the loadAdditionalDetails call:
...
async function loadAdditionalDetails(details) {
    try {
        ...
        return `
            <div class="flex gap-4 details-container">
                <div class="bg-white text-gray-700 p-4 rounded-lg shadow dark:bg-gray-700 dark:text-white w-full">
                    <h4 class="font-semibold text-xl text-gray-700 dark:text-white mb-2">Route Information</h4>
                    <div class="space-y-2 text-sm">
                        <div class="mapouter">
                            <div class="gmap_canvas">
                                <iframe width="100%" height="340" src="https://maps.google.com/maps?q=${details.pickup.split(', ').slice(0, 3).join(',')}to=${details.dropoff.split(', ').slice(0, 3).join(',')}&t=&z=7&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe>
                            </div>
                        </div>
                    </div>
                </div>
